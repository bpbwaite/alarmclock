...
void alck::timingFunction() {
    ...
    clockDisplay->showNumberDecEx(timeReadyToShow, !alarmIsSet || (millis() % 2000 > 1000 && !m_mode) ? mask : 0x0, (timeReadyToShow < 99U) || m_mode, 0x4, 0x0);
}
...
void alck::brightnessHandlerRoutine() {
    ...
    clockDisplay->setBrightness(maskClip(ceil(0.0003 * pow(t_dec, 4) - 0.0182 * pow(t_dec, 3) + 0.316 * pow(t_dec, 2) - 1.212 * t_dec + 0.972)));
}
void alck::execute() {
    timingFunction();
    alarmingFunction();
